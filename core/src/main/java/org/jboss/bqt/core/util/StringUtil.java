...
public final class StringUtil {
	 ...
	 * {@link Constants#NEW_LINE}
	 */
	public static final String LINE_SEPARATOR = System.getProperty(
			LINE_SEPARATOR_PROPERTY_NAME, Constants.NEW_LINE);
	...
	/**
	 * Utility to return a string enclosed in ''. Creation date: (12/2/99
	 * 12:05:10 PM)
	 * @param aString 
	 * @return String
	 */
	...
	/**
	 * Join string pieces and separate with a delimiter. Similar to the perl
	 * function of the same name. If strings or delimiter are null, null is
	 * returned. Otherwise, at least an empty string will be returned.
	 * 
	 * @see #split
	 * 
	 * @param strings
	 *            String pieces to join
	 * @param delimiter
	...
	public static String join(List strings, String delimiter) {
		...
		// This is the standard problem of not putting a delimiter after the
		// last
		// string piece but still handling the special cases. A typical way is
		// to check every
		// iteration if it is the last one and skip the delimiter - this is
		// avoided by
		// looping up to the last one, then appending just the last one.

		// First we loop through all but the last one (if there are at least 2)
		// and
		// put the piece and a delimiter after it. An iterator is used to walk
		// the list.
		...
		// If there is at least one element, put the last one on with no
		// delimiter after.
		if (strings.size() > 0) {
	...
	/**
	 * Break a string into pieces based on matching the full delimiter string in
	 * the text. The delimiter is not included in the returned strings.
	 * 
	 * @param target
	 *            The text to break up.
	 * @param delimiter
	...
	public static String replace(String source, String search, String replace) {
		if (source != null && search != null && search.length() > 0
			...
			if (start > -1) {
				return new StringBuffer(source).replace(start,
						start + search.length(), replace).toString();
			}
		}
		return source;
	}
	...
	/*
	 * Replace all occurrences of the search string with the replace string in
	 * the source string. If any of the strings is null or the search string is
	 * zero length, the source string is returned.
	 * 
	 * @param source the source string whose contents will be altered
	 * 
	 * @param search the string to search for in source
	...
	public static void replaceAll(StringBuffer source, String search,
			String replace) {
		if (source != null && search != null && search.length() > 0
				&& replace != null) {
			...
			while (start > -1) {
				...
				start = source.toString().indexOf(search,
	 ...
	 * 
	 * @param in
	 *            the string that may need tuncating.
	 * @param len
	...
	/**
	 * Simple utility method to wrap a string by inserting line separators
	 * creating multiple lines each with length no greater than the user
	 * specified maximum. The method parses the given string into tokens using a
	 * space delimiter then reassembling the tokens into the resulting string
	 * while inserting separators when required. If the number of characters in
	 * a single token is greater than the specified maximum, the token will not
	 * be split but instead the maximum will be exceeded.
	 * 
	 * @param str
	 *            the string that may need tuncating.
	 * @param maxCharPerLine
	 *            the max number of characters per line
	 * @return a new String containing line separators or the original string if
	 *         its length was less than the maximum.
	 */
	public static String wrap(String str, int maxCharPerLine) {
		int strLength = str.length();
		if (strLength > maxCharPerLine) {
			StringBuffer sb = new StringBuffer(str.length()
					+ (strLength / maxCharPerLine) + 1);
			strLength = 0;
	...
	/**
	 * Return the tokens in a string in a list. This is particularly helpful if
	 * the tokens need to be processed in reverse order. In that case, a list
	 * iterator can be acquired from the list for reverse order traversal.
	 * 
	 * @param str
	...
	/**
	 * Return the number of tokens in a string that are seperated by the
	 * delimiter.
	 * 
	 * @param str
	 *            String to be tokenized
	...
	/**
	 * Return the number of occurrences of token string that occurs in input
	 * string. Note: token is case sensitive.
	 * 
	 * @param input
	 * @param token
	 * @return int
	 ...
	 * characters and that are separated by a single space. For example, the
	 * following are the outputs of some sample inputs: <li>"aName" is converted
	 * to "A Name"</li> <li>"Name" is converted to "Name"</li> <li>"NAME" is
	 * converted to "NAME"</li> <li>"theName" is converted to "The Name"</li>
	 * <li>"theBIGName" is converted to "The BIG Name"</li> <li>"the BIG Name"
	 * is converted to "The BIG Name"</li> <li>"the big Name" is converted to
	 * "The Big Name"</li> <li>"theBIG" is converted to "The BIG"</li> <li>
	 * "SQLIndex" is converted to "SQL Index"</li> <li>"SQLIndexT" is converted
	 * to "SQL Index T"</li> <li>"SQLIndex T" is converted to "SQL Index T"</li>
	...
	public static String computeDisplayableForm(String str, String defaultValue) {
		...
		if (newName.length() > 1) {
			...
			for (int i = 1; i != newName.length(); ++i) {
				...
				// If the previous character is a space, capitalize the current
				// character
				if (prevChar == ' ') {
				...
				// Otherwise, if the current character is already uppercase ...
					...
					// ... and the previous character is not uppercase, then
					// insert
					if (!previousCharUppercase) {
						...
						if (currentChar != 'M'
								...
								|| (!newName
										...
										.startsWith(
												CorePlugin.Util
														.getString("StringUtil.Displayable")))) { //$NON-NLS-1$
							newName.insert(i, ' ');
					...
					// ... and the previous character is uppercase ...
						...
						// ... but the next character neither uppercase or a
						// space ...
	 ...
	 * @param str
	 *            any string.
	 * @return if the str argument occurs as a substring within text, then the
	 *         index of the first character of the first such substring is
	 *         returned; if it does not occur as a substring, <code>-1</code> is
	 *         returned. If the text or str argument is null or empty then
	 ...
	 *         <code>true</code> will be returned if the prefix is an empty
	 *         string or is equal to the text <code>String</code> object as
	 *         determined by the {@link #equals(Object)} method. If the text or
	 ...
	 *         will be <code>true</code> if the suffix is the empty string or is
	 *         equal to this <code>String</code> object as determined by the
	 *         {@link #equals(Object)} method. If the text or suffix argument is
	 *         null <code>false</code> is returned.
	 */
	public static boolean endsWithIgnoreCase(final String text,
		...
		return text.regionMatches(true, text.length() - suffix.length(),
	...
	static String asRegEx(String pattern) {
		...
		String result1 = Constants.PATTERN_BACK_SLASH.matcher(pattern)
				.replaceAll("\\\\E\\\\\\\\\\\\Q"); //$NON-NLS-1$
		String result2 = Constants.PATTERN_STAR.matcher(result1).replaceAll(
				"\\\\E.*\\\\Q"); //$NON-NLS-1$
		String result3 = Constants.PATTERN_QUESTION.matcher(result2)
				.replaceAll("\\\\E.\\\\Q"); //$NON-NLS-1$
	 ...
	 * our old 'StringMatcher' format). Copied from Eclipse's PatternConstructor
	 * class.
	 * 
	 * @param pattern
	 *            The search pattern
	...
	public static String collapseWhitespace(String raw) {
		...
		while (izer.hasMoreTokens()) {
			...
			// Added one last check here so we don't append a "space" on the end
			// of the string
			rv.append(tok);
	...
	/**
	 * If input == null OR input.length() < desiredLength, pad to desiredLength
	 * with spaces. If input.length() > desiredLength, chop at desiredLength.
	 * 
	 * @param input
	 *            Input text
	 * @param desiredLength
