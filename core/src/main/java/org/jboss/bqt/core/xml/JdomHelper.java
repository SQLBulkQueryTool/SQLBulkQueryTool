...
public class JdomHelper {
	...
	// private static String SCHEMA_NAMESPACE =
	 ...
	 */
	public static Document buildDocument(String filename) throws IOException,
			JDOMException {
		return buildDocument(SAXBuilderHelper.getParserClassName(), filename,
				DEFAULT_VALIDATION);
	}
	 ...
	 *             when errors occur in parsing.
	 */
	public static Document buildDocument(File file) throws IOException,
			JDOMException {
		return buildDocument(SAXBuilderHelper.getParserClassName(), file,
				DEFAULT_VALIDATION);
	}
	 ...
	 *            <code>String</code> name of file.
	 * @param validateXML
	 *            indicates whether to validate the XML document using the DTD
	 ...
	 *             when errors occur in parsing.
	 */
	public static Document buildDocument(String filename, boolean validateXML)
	 ...
	 *            <code>String</code> name of driver class to use.
	 * @param filename
	 *            <code>String</code> name of file.
	 * @param validateXML
	 *            indicates whether to validate the XML document using the DTD
	 ...
	 *             when errors occur in parsing.
	 */
	public static Document buildDocument(String saxDriverClass,
			String filename, boolean validateXML) throws IOException,
			JDOMException {
		if (filename == null) {
			ArgCheck.isNotNull(filename, CorePlugin.Util
					.getString("JdomHelper.The_file_name_may_not_be_null_3")); //$NON-NLS-1$
		}

		if (filename.length() == 0) {
			ArgCheck.isNotZeroLength(
					filename,
					CorePlugin.Util
							.getString("JdomHelper.The_file_name_may_not_be_zero-length_4")); //$NON-NLS-1$
		}

		SAXBuilder builder = SAXBuilderHelper.createSAXBuilder(saxDriverClass,
				validateXML);
	 ...
	 *             when errors occur in parsing.
	 */
	public static Document buildDocument(String saxDriverClass, File file,
			boolean validateXML) throws IOException, JDOMException {
		SAXBuilder builder = SAXBuilderHelper.createSAXBuilder(saxDriverClass,
				validateXML);
	 ...
	 * 
	 * @param stream
	 *            the input stream from which the document is to be read
	 * @return Document
	 * @throws IOException
	 *             if the file does not exist.
	 * @throws JDOMException
	...
	public static Document buildDocument(InputStream stream)
			throws IOException, JDOMException {
		return buildDocument(SAXBuilderHelper.getParserClassName(), stream,
				DEFAULT_VALIDATION);
	}
	 ...
	 *             when errors occur in parsing.
	 */
	public static Document buildDocument(InputStream stream, boolean validateXML)
			throws IOException, JDOMException {
		return buildDocument(SAXBuilderHelper.getParserClassName(), stream,
				validateXML);
	}
	 ...
	 */
	public static Document buildDocument(String saxDriverClass,
			InputStream stream, boolean validateXML) throws IOException,
			JDOMException {
		if (stream == null) {
			ArgCheck.isNotNull(stream, CorePlugin.Util
					.getString("JdomHelper.The_InputStream_may_not_be_null_7")); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @param stream
	 *            the Reader from which the document is to be read
	 * @return Document
	 * @throws IOException
	 *             if the file does not exist.
	 * @throws JDOMException
	 *             when errors occur in parsing.
	 */
	public static Document buildDocument(java.io.Reader stream)
			throws IOException, JDOMException {
		return buildDocument(SAXBuilderHelper.getParserClassName(), stream,
				DEFAULT_VALIDATION);
	}
	 ...
	 */
	public static Document buildDocument(String saxDriverClass,
			java.io.Reader stream, boolean validateXML) throws IOException,
			JDOMException {
		if (stream == null) {
			ArgCheck.isNotNull(stream, CorePlugin.Util
					.getString("JdomHelper.The_InputStream_may_not_be_null_10")); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @param rootTag
	 *            the name of the root tag for the new document.
	 * @return Document
	 *            </p>
	 */
	public static Document createNewDocument(String rootTag) {
		if (rootTag == null) {
			ArgCheck.isNotNull(
					rootTag,
					CorePlugin.Util
							.getString("JdomHelper.The_root_tag_name_may_not_be_null_11")); //$NON-NLS-1$
		}
		...
		if (rootTag.length() == 0) {
			ArgCheck.isNotZeroLength(
					rootTag,
					CorePlugin.Util
							.getString("JdomHelper.The_root_tag_name_may_not_be_zero-length_12")); //$NON-NLS-1$
		}
	...
	/**
	 * <p>
	 * This returns a List of Element objects that are descendents of this
	 * element and within the specified Namespace. If this target element has no
	 * nested elements within this namespace, an empty List is returned. The
	 * returned list is "live" and changes to it affect the element's actual
	 * contents.
	 * </p>
	 * 
	 * @param traversalMethod
	 *            the method used to traverse the JDOM tree.
	 * @param parent
	 *            the JDOM <code>Element</code> to get descendents for.
	 * @param ns
	...
	public static List getDescendents(int traversalMethod, Element parent,
		...
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_13")); //$NON-NLS-1$
		}
		if (ns == null) {
			ArgCheck.isNotNull(
					ns,
	 ...
	 * <p>
	 * This returns a List of Element objects that are descendents of this
	 * element. If this target element has no nested elements, an empty List is
	 ...
	 *            the JDOM <code>Element</code> to get descendents for.
	 * @param name
	 *            tag name for the descendents to match
	 * @return list of descendent JDOM <code>Element</code>s or an empty list if
	 *         none exist.
	 */
	public static List getDescendents(Element parent, final String name) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_16")); //$NON-NLS-1$
		}

		if (name == null) {
			ArgCheck.isNotNull(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_null_17")); //$NON-NLS-1$
		}

		if (name.length() == 0) {
			ArgCheck.isNotZeroLength(name, CorePlugin.Util
	 ...
	 * <p>
	 * This returns a List of all elements that are descendents of this element
	 * belonging to the specified Namespace, returned as Element objects. If
	 * this target element has no nested elements within this Namespace, an
	 * empty List is returned. The returned list is "live" and changes to it
	...
	public static List getDescendents(Element parent, final Namespace ns) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_19")); //$NON-NLS-1$
		}
		if (ns == null) {
			ArgCheck.isNotNull(ns, CorePlugin.Util
	 ...
	 * <p>
	 * This returns a List of all elements that are descendents of this element
	 * with the given local name and belonging to the specified Namespace,
	 * returned as Element objects. If this target element has no nested
	 * elements with the given name in the specified Namespace, an empty List is
	 * returned. The returned list is "live" and changes to it affect the
	 * element's actual contents.
	 * </p>
	 * 
	...
	public static List getDescendents(Element parent, final String name,
		...
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_21")); //$NON-NLS-1$
		}

		if (name == null) {
			ArgCheck.isNotNull(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_null_22")); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @param parent
	 *            the JDOM <code>Element</code> to use as the root for a count.
	...
	public static int getDescendentCount(Element parent) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_24")); //$NON-NLS-1$
		}
	 ...
	 * <p>
	 * This returns the number of elements that are descendents of this element
	 * and belonging to the specified Namespace. The resulting count excludes
	...
	public static int getDescendentCount(Element parent, final Namespace ns) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_25")); //$NON-NLS-1$
		}
	 ...
	 * <p>
	 * This returns the number of elements that are descendents of this element
	 * with the given local name. The resulting count excludes the specified
	 * parent Element.
	 * </p>
	 * 
	 * @param parent
	...
	public static int getDescendentCount(Element parent, final String name) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_26")); //$NON-NLS-1$
		}

		if (name == null) {
			ArgCheck.isNotNull(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_null_27")); //$NON-NLS-1$
		}

		if (name.length() == 0) {
			ArgCheck.isNotZeroLength(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_zero-length_28")); //$NON-NLS-1$
		}
	...
	/**
	 * <p>
	 * This returns the first element that is a descendent of this element and
	 * matches given local name.
	 * </p>
	 * 
	 * @param parent
	 *            the JDOM <code>Element</code> to check descendents for.
	 * @param name
	 *            tag name for the descendents to match
	...
	public static Element findElement(Element parent, final String name) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_29")); //$NON-NLS-1$
		}

		if (name == null) {
			ArgCheck.isNotNull(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_null_30")); //$NON-NLS-1$
		}

		if (name.length() == 0) {
			ArgCheck.isNotZeroLength(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_zero-length_31")); //$NON-NLS-1$
		}
	 ...
	 * <p>
	 * This returns the first element that is a descendent of this element and
	 * matches the specified local name and Namespace.
	 * </p>
	 * 
	 * @param parent
	 *            the JDOM <code>Element</code> to check descendents for.
	...
	public static Element findElement(Element parent, final String name,
		...
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_32")); //$NON-NLS-1$
		}

		if (name == null) {
			ArgCheck.isNotNull(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_null_33")); //$NON-NLS-1$
		}

		if (name.length() == 0) {
			ArgCheck.isNotZeroLength(name, CorePlugin.Util
					.getString("JdomHelper.The_name_may_not_be_zero-length_34")); //$NON-NLS-1$
		}
		...
		if (ns == null) {
			ArgCheck.isNotNull(
					ns,
					CorePlugin.Util
							.getString("JdomHelper.The_Namespace_reference_may_not_be_null_35")); //$NON-NLS-1$
		}
	 ...
	 * node during the traversal. This is equivalent to a <i>breadth first
	 * search</i>.
	 * </p>
	 * 
	 * @param parent
	 *            the JDOM <code>Element</code> to use as the root for the
	 *            traversal.
	 * @param v
	 *            the <code>XMLVisitor</code> to apply at each node as the tree
	 *            is traversed.
	 */
	public static void levelOrderTraversal(Element parent, XMLVisitor v) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_36")); //$NON-NLS-1$
		}
		if (v == null) {
			ArgCheck.isNotNull(
					v,
	...
	public static void preOrderTraversal(Element parent, XMLVisitor v) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_38")); //$NON-NLS-1$
		}
		if (v == null) {
			ArgCheck.isNotNull(
					v,
	...
	public static void postOrderTraversal(Element parent, XMLVisitor v) {
		if (parent == null) {
			ArgCheck.isNotNull(
					parent,
					CorePlugin.Util
							.getString("JdomHelper.The_JDOM_Element_reference_may_not_be_null_40")); //$NON-NLS-1$
		}
		if (v == null) {
			ArgCheck.isNotNull(
					v,
	 ...
	 * @param filename
	 *            the fully qualified name of the file to output the XML to.
	 * @throws IOException
	...
	/**
	 * <p>
	 * Ouput the current JDOM <code>Document</code> to a file with the specified
	 * name.
	 * </p>
	 * 
	 * @param doc
	 ...
	 *            the fully qualified name of the file to output the XML to.
	 * @param indent
	 *            the indent String, usually some number of spaces
	 * @param newlines
	 *            true indicates new lines should be printed, else new lines are
	 *            ignored (compacted).
	 * @throws IOException
	 *             if there are problems writing to the file.
	 */
	public static void write(Document doc, String filename, String indent,
			boolean newlines) throws IOException {
		ArgCheck.isNotNull(
				doc,
				CorePlugin.Util
						.getString("JdomHelper.The_Document_reference_may_not_be_null_42")); //$NON-NLS-1$
		ArgCheck.isNotNull(filename, CorePlugin.Util
				.getString("JdomHelper.The_filename_may_not_be_null_43")); //$NON-NLS-1$

		if (filename.length() == 0) {
			ArgCheck.isNotZeroLength(
					filename,
					CorePlugin.Util
							.getString("JdomHelper.The_filename_may_not_be_zero-length_44")); //$NON-NLS-1$
		}
	 ...
	 *             if there are problems writing to the file.
	 */
	public static void write(Document doc, OutputStream stream)
			throws IOException {
	...
	// */
	// public static void write( Document doc, OutputStream stream, String
	// indent,
	...
	public static void write(Document doc, OutputStream stream, String indent,
		...
		if (doc == null) {
			ArgCheck.isNotNull(
					doc,
					CorePlugin.Util
							.getString("JdomHelper.The_Document_reference_may_not_be_null_45")); //$NON-NLS-1$
		}
		if (stream == null) {
			ArgCheck.isNotNull(
					stream,
					CorePlugin.Util
							.getString("JdomHelper.The_OutputStream_reference_may_not_be_null_46")); //$NON-NLS-1$
		}
	 ...
	 *             if there are problems writing to the file.
	 */
	public static void write(Document doc, Writer writer, String indent,
			boolean newlines) throws IOException {
		if (doc == null) {
			ArgCheck.isNotNull(
					doc,
					CorePlugin.Util
							.getString("JdomHelper.The_Document_reference_may_not_be_null_47")); //$NON-NLS-1$
		}
		if (writer == null) {
			ArgCheck.isNotNull(
					writer,
					CorePlugin.Util
							.getString("JdomHelper.The_Writer_reference_may_not_be_null_48")); //$NON-NLS-1$
		}
		XMLOutputter outputter = new XMLOutputter(getFormat(indent, newlines));
	 ...
	 *             if there are problems writing to the file.
	 */
	public static String write(Document doc, String indent, boolean newlines)
			throws IOException {
		if (doc == null) {
			ArgCheck.isNotNull(
					doc,
					CorePlugin.Util
							.getString("JdomHelper.The_Document_reference_may_not_be_null_49")); //$NON-NLS-1$
		}
		XMLOutputter outputter = new XMLOutputter(getFormat(indent, newlines));
	...
	public static void print(PrintStream stream, Document doc) {
		if (stream == null) {
			ArgCheck.isNotNull(
					stream,
					CorePlugin.Util
							.getString("JdomHelper.The_stream_reference_may_not_be_null_50")); //$NON-NLS-1$
		}
		stream.println(CorePlugin.Util
				.getString("JdomHelper.JDOM_Document_tree_51")); //$NON-NLS-1$
		Element root = doc.getRootElement();
	...
	/**
	 * Get the best content value for a JDOM object. For elements, the content
	 * text is returned. For attributes, the attribute value is returned. For
	 * namespaces, the URI is returned. Etc...
	 * 
	 * @param jdomObject
	 *            JDOM object such as Element, Attribute, Text, Namespace,
	 *            Comment, ProcessingInstruction, String
