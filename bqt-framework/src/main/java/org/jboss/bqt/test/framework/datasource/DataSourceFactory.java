...
/**
 * The DataSourceFactory is responsible for managing the datasources used during
 * a single test. It ensures the same data source is used in both the connector
 * binding and during validation to ensure validation is performed against the
 * same datasource the test was performed on.
 * 
 * The following are the available options for controlling which datasources are
 * used during a test: <li>Control which datasources are used and to which model
 * they are assigned</li>
 * 
 * <p>
 * Use the {@link ConfigPropertyNames#USE_DATASOURCES_PROP} property to specify
 * a comma delimited ordered list to control which data sources to use.
 * 
 * <br>
 * <br>
 * This will enable integration testing to be setup to use well known
 * combinations, to ensure coverage and makes it easier to replicate issues.
 * 
 * This indicates to use only the specified datasources for this test. This
 * option can be used in test cases where only specific sources are to be used
 * and tested against.
 * 
 * This ordered list will map to the model:order mapping in the
 * config.properties file. <br>
 * <br>
 * Example: in the config.properties file, the models in the test and their
 ...
 * 
 * This will use oracle and sqlserver datasources in the test, and when a
 * datasource is requested for model "pm1", the oracle datasource will be
 * returned. And when a data source for "pm2" is requested, the data source
 * mapped to 2nd ordered datasource of sqlserver will be returned.
 * 
 * </p>
 * <li>Control which datasources of a specific database type to exclude</li>
 * <p>
 * 
 * Use the {@link ConfigPropertyNames#EXCLUDE_DATASBASE_TYPES_PROP} property to
 * specify a comma delimited list of {@link DataBaseTypes} to exclude. <br>
 * <br>
 ...
 * {@link ConfigPropertyNames#USE_DATASOURCES_PROP} option. This is done because
 * there are some test that will not work with certain database types, and
 * therefore, if the required datasources are not met, the test will be bypassed
 * for execution.
 * </p>
 * <li>Control for a specfic model, which database type of datasource to be
 * assigned</li>
 * <p>
 * This option gives the developer even more fine grain control of how
 * datasources are assigned. There are cases where a specific model must be
 * assigned to a datasource of a specific database type.
 * 
 * To use this option,
 * 
 * </p>
...
public class DataSourceFactory {
	...
	/**
	 * These types match the "ddl" directories for supported database types and
	 * it also found in the datasources connection.properties defined by the
	 * DB_TYPE property These are also the values used to specify the required
	 * database types.
	 * 
	 */
	...
	// the DO_NO_USE_DEFAULT will be passed in when the test are run from maven
	// and no property is passed in for UseDataSources
	private static final String DO_NOT_USE_DEFAULT = "${"
	...
	// contains any dbtype preconditions on a model, which requires a model to
	// be assigned a certain database type
	...
	// indicates if the datasource requirements have been, it will be false in
	// the case
	// a specific dbtype is required and that type was not one of the available
	// types defined
	...
	public DataSourceFactory(ConfigPropertyLoader config) {
		this.configprops = PropertiesUtils.clone(config.getProperties(), null,
				true);
	 ...
	 * 
	 * 
	 * 1st, check for the usedatasource property, if exist, then only add those
	 * specific datasources to the useDataSources, otherwise add all available.
	 * 2nd, if the exclude option is used, then remove any excluded datasources
	 * from the useDataSources.
	 * @param dsmgr 
	 */
	protected void config(DataSourceMgr dsmgr) {
		...
		if (limitdsprop != null && limitdsprop.length() > 0
			...
			for (Iterator<DataSource> it = availDatasources.values().iterator(); it
					.hasNext();) {
				DataSource ds = it.next();
				// if the datasource type is not excluded, then consider for
				// usages
				if (!excludedDBTypes.contains(ds.getDBType())) {

					// if use a specific db type is specified, then it must
					// match,
		...
		if (requiredDataBaseTypes != null && requiredDataBaseTypes.size() > 0) {
			...
			Iterator<String> rit = this.requiredDataBaseTypes.keySet()
					.iterator();
			...
			while (rit.hasNext()) {
				...
				// go thru all the datasources and find the matching datasource
				// of the correct dbtype
				while (ait.hasNext()) {
	...
	public synchronized DataSource getDatasource(String modelName)
		...
		if (this.hasRequiredDBTypes) {
			if (this.requiredDataBaseTypes.containsKey(modelName)) {
				...
				// need to go thru all the datasources to know if any has
				// already been
				// assigned
			...
			// need to go thru all the datasources to know if any has already
			// been
			// assigned
			// because the datasourceid passed in was a group name
			while (it.hasNext()) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (dsfind == null) {
				throw new TransactionRuntimeException(
						"No datasource was not found as the 2nd datasource");

			}

			if (dsfind.getConnectorType() == null) {
				throw new TransactionRuntimeException(
						"Connector types was not defined");
			}

			if (!dsfind.getName().equalsIgnoreCase("sqlserver")) {
				throw new TransactionRuntimeException(
						"Sqlserver was not found as the 2nd datasource");
			...
			if (!dsfind.getName().equalsIgnoreCase("oracle")) {
				throw new TransactionRuntimeException(
						"Oracle was not found as the 2nd datasource");

			}
			System.out
					.println("Datasource :" + dsfind.getName() + " was found");
			...
			config = ConfigPropertyLoader.getInstance();
			config.setProperty(
					ConfigPropertyNames.EXCLUDE_DATASBASE_TYPES_PROP,
					"sqlserver");
			...
			for (int i = 0; i < n; i++) {
				...
				if (ds1 == null) {
					throw new TransactionRuntimeException(
							"No datasource was found for: model:" + k);

				}
				if (ds1.getDBType().equalsIgnoreCase(
						DataSourceFactory.DataBaseTypes.SQLSERVER)) {
					throw new TransactionRuntimeException(
							"sqlserver dbtype should have been excluded");
				}
			}
			...
			if (reuse != null) {
				...
				throw new TransactionRuntimeException(
						"The process was not able to reassign an already used datasource");
			...
			if (!ds1.getDBType().equalsIgnoreCase(
					DataSourceFactory.DataBaseTypes.ORACLE)) {
				throw new TransactionRuntimeException(
						"Required DB Type of oracle for model pm1 is :"
								+ ds1.getDBType());
			}
			...
			if (!ds2.getDBType().equalsIgnoreCase(
					DataSourceFactory.DataBaseTypes.SQLSERVER)) {
				throw new TransactionRuntimeException(
						"Required DB Type of sqlserver for model pm2 is :"
								+ ds2.getDBType());
			}
			...
			if (!ds3a.getDBType().equalsIgnoreCase(
					DataSourceFactory.DataBaseTypes.SQLSERVER)) {
				throw new TransactionRuntimeException(
						"Required DB Type of sqlserver for model pm12 is :"
								+ ds3a.getDBType());
			}

			DataSource ds3b = factory.getDatasource("pm1");
			if (!ds3b.getDBType().equalsIgnoreCase(
					DataSourceFactory.DataBaseTypes.ORACLE)) {
				throw new TransactionRuntimeException(
						"Required DB Type of oracle for model pm1  is :"
								+ ds3b.getDBType());
