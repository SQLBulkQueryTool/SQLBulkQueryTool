...
import org.jboss.bqt.client.xml.ResultsHolder;
import org.jboss.bqt.core.util.SqlUtil;
...
public class ExpectedResultsImpl implements ExpectedResults {
	...
	public ExpectedResultsImpl(String querySetIdentifier, Properties properties) {
		...
		ClientPlugin.LOGGER.info("Expected results loc: " + this.results_dir_loc);
	}
	...
	@Override
	public Object compareResults(TestResult testresults, ResultSet resultSet,
			boolean isOrdered, boolean resultFromQuery)
			throws QueryTestFailedException {

		File expectedResultsFile = getResultsFile(testresults.getQueryID());

		List<?> results = null;
		if (testresults.getException() != null) {
			try {
				results = TestResultSetUtil.compareThrowable(testresults.getException(),
						testresults.getQuery(), expectedResultsFile, false);
		...
		if (!resultFromQuery) {
			...
			if (SqlUtil.isUpdateSql(testresults.getQuery())) {
				if (testresults.getUpdateCount() == 0 && expectedResultsFile.length() > 0) {
					throw new QueryTestFailedException("Update cnt was zero: "
							+ expectedResultsFile.getName());
				}
				if (testresults.getUpdateCount() > 0 && expectedResultsFile.length() == 0) {
				...
				if (expectedResultsFile.length() > 0) {
					throw new QueryTestFailedException(
							"No results from query, but expected results");
				}
			}
			...
			try {
				...
				results = TestResultSetUtil.writeAndCompareResultSet(resultSet,
						testresults.getQuery(), MAX_COL_WIDTH, false, null, expectedResultsFile,
						false);
