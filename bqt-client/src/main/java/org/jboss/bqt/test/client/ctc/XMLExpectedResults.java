...
import org.jboss.bqt.test.client.ClassFactory;
import org.jboss.bqt.test.client.ExpectedResults;
import org.jboss.bqt.test.client.QueryScenario;
import org.jboss.bqt.test.client.QueryTest;
...
public class XMLExpectedResults implements ExpectedResults {
	...
	public XMLExpectedResults(String querySetIdentifier, Properties properties) {
		...
		if (EXCEED_PER != null && EXCEED_PER.trim().length() > 0) {
			TestLogger.logDebug(" ======== " + TestProperties.PROP_EXECUTE_EXCEED_PERCENT + " is set to " + EXCEED_PER);
	...
	protected void compareResults(final ResultsHolder actualResults,
		...
		if (this.exec_minumin_time > 0 && e > this.exec_minumin_time &&  a > e) {
				...
				TestLogger.log("EXEC MIN TIME: " + this.exec_minumin_time + "  EXEC PER: " + exceed_percent + "  expected exec time: " + expectedResults.getExecutionTime());
				TestLogger.log("   expected exec time: " + e + "  actual exec time: " + a);
				if ( ( a - allowediff) > e ) {
